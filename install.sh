...
genDesktopFile() {
    ...
    name=$1
    sufix="${name}-nix"
    binPath=$(runNix echo "which ${name}")
    ...
    TRASH=$(array_append "$TRASH" ~/.local/share/applications/${sufix}.desktop ~/.local/share/icons/${name}.svg)
}
...
installFonts() {
    ...
    TRASH=$(array_append "$TRASH" $(readlink -f $file) ${fonts_dir}/{README.txt,specimen.html})
    fc-cache -f
}
